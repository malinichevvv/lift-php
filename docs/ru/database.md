...
>
> **Начиная с 1.3.0:** `update()` и `delete()` отказываются выполняться без `WHERE`. Для намеренной операции по всей таблице сначала вызовите `allowMassUpdate()`, `allowMassDelete()` или `allowMassMutation()`.
...
| Огромный `UPDATE` выполнился без `WHERE` | Вы забыли `->where(...)` | Lift теперь отказывается выполнять это по умолчанию; добавьте `where` или явный `allowMassUpdate()`. |

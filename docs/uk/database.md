...
>
> **Починаючи з 1.3.0:** `update()` і `delete()` відмовляються виконуватися без `WHERE`. Для навмисної операції по всій таблиці спочатку викличте `allowMassUpdate()`, `allowMassDelete()` або `allowMassMutation()`.
...
| Величезний `UPDATE` виконався без `WHERE` | Ви забули `->where(...)` | Lift тепер відмовляється виконувати це за замовчуванням; додайте `where` або явний `allowMassUpdate()`. |
